...
def run_load_to_db():
    ...
    STATIONS_PATH = os.path.join(BASE_DIR, config["stations_path"])
    STATION_ID_PATH = os.path.join(BASE_DIR, config["station_id_path"])
    ...
    for _, row in station_df.iterrows():
        cur.execute(
            """
            INSERT INTO stations (station_name, aus_state, latitude, longitude, elevation_m, starting_date, end_date, coverage_pct)
            ...
            (
                ...
                row.get("longitude"),
                row.get("elevation_m"),
                clean_date(row.get("starting_date")),
                clean_date(row.get("end_date")),
    ...
    insert_sql = """
        INSERT INTO daily_weather
            (station_id, obs_date, evapotranspiration_mm, rain_mm, max_temp_c, min_temp_c,
