...
no_issues = sum(df.issue == "No")
stopped = sum(df.issue == "Station Stopped")
not_enough = sum(df.issue == "Not enough data")
coverage = sum(df.issue == "Coverage")

print(f"\tStations without issues: {no_issues}")
print(f"\tStations stopped working: {stopped}")
print(f"\tStations not having enough data: {not_enough}")
print(f"\tStations with coverage issue: {coverage}")
